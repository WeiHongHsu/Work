	Declare 
		...
		,@col25 nvarchar(400)
			...
			and cs.container_id_original = 'ESL-202308308602401'
			--select * from iexp..W_Case_S (nolocK) where customer_code = 'REY'
	...
	select REPLACE(@Json_Result,']','')

	--Declare @outputJson as NVARCHAR(MAX)
	--Declare @apiStatus as INT
	--Declare @apiMessage as NVARCHAR(500)
	--Exec SP_GetShip_FAMI @Json_Result,@outputJson output,@apiStatus output,@apiMessage output
