...
Public NotInheritable Class DBConCls
	'-- 中間庫位置
	Public Shared EDIconstr As String = "Data Source=192.168.1.121;Persist Security Info=True;Password=zaq1;User ID=sa;Initial Catalog=iexp"
	Public Shared CONNs As String = "Data Source=192.168.1.121;Persist Security Info=True;Password=zaq1;User ID=sa;Initial Catalog=SAPi"
	'--EDI JobQ 位置
	Public Shared JobQConstr As String = "Data Source=LOCALHOST;Persist Security Info=True;Password=zaq1;User ID=sa;Initial Catalog=SJOB"
	...
	Public Shared Function SQLXmlRW(ByVal sqlstr As String, ByVal expPath As String, ByRef its As Integer) As String
		...
		Try
			Using con As New SqlConnection(EDIconstr)
				...
				doc.Load(RXml)
				'Dim xr As XmlReader = XmlReader.Create(New StringReader(doc.OuterXml))
				'ds.ReadXml(xr)
				'ds.ReadXml(XmlReader.Create(New StringReader(doc.OuterXml)))
				'doc.InsertBefore(doc.CreateXmlDeclaration("1.0", "UTF-8", Nothing), doc.DocumentElement)
				' 驗證SQLXML 是否格式正常；筆數
				'For i = 0 To ds.Tables.Count - 1
				'	its += ds.Tables(i).Rows.Count
				'Next
