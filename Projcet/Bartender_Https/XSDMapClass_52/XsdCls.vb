...
#Region "類別建構式"
	...
	Public Sub New(ByVal iInf As String, ByVal iwkey As String, ByVal iStkey As String, ByVal iXsdFile As String)
		...
		Try
			...
			Else
				'TODO 20220412 Raines change connP
				'PASSWORD = vs.com.Readini.INIReader.getValue("D:\SJOB\SQLCLR\Eslite.ini", "All", "PASSWORD")
				'DBConCls.CONNs = DBConCls.EDIconstr.Replace("@Password", "pstG)MM*NYJcVh*FQ830!A!j")
				DBConCls.EDIconstr = DBConCls.CONNs
				'FileIO.FileSystem.WriteAllText(APPLog, PASSWORD, True)
			End If
			...
			BulkBatchRow = IIf(XApi.<EDIinf>.@BulkBatchRow Is Nothing, 1000, XApi.<EDIinf>.@BulkBatchRow)
...
#End Region
	...
	Public Function ErrFmtLog(ByVal Pr() As String) As String
		ErrFmtLog = ""
		Try
			Dim ErrFmt As String = Now.ToString("yyyy/MM/dd hh:mm:ss") & vbTab & "{0,-25}	{1,-20}	{2,-20}	ErrMessage:{3,-30} / {4,-50}" & vbCrLf
	...
	Public Function XsdMapCopy() As String
		...
		If BulkBatchRow <= 0 Then BulkBatchRow = 1
		SqlCopy.BatchSize = BulkBatchRow
		...
		For Each vt In Vxsd
			...
			For Each elx In MapInf
				...
				If Mds.Tables(vt.name).Columns.Contains(elx.Tfield) = True And elx.Sqlfield.ToString <> "" Then
			...
			If Mds.Tables(vt.name).Rows.Count > 0 And SqlCopy.ColumnMappings.Count > 0 Then
				...
				Catch ex As Exception
					...
					Dim Pr() As String = {"XsdMapCopy(...)", SqlCopy.DestinationTableName, "欄數:" & SqlCopy.ColumnMappings.Count & "+筆數:" & Mds.Tables(vt.name).Rows.Count, "<SqlCopy: WriteToServer錯誤>BatchRow" & BulkBatchRow, ex.Message}
