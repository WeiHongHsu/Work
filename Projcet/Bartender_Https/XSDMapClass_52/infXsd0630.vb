...
Public Class infXsdFun
	...
	Public Shared Function XsdBulkCopy(ByVal ikey As SqlString, ByVal iwkey As SqlString, ByVal iStkey As SqlString, ByVal InfXSD As SqlString) As SqlString
		...
		Try
			...
			'====取得: xsd:appinf 參數
			Extkeys = iStkey.Value.Split(New Char() {","})
			Stkey = Extkeys(0)
			InfKey = ikey.Value
			...
			''' 傳入檔案的目錄選項 
			If iwkey.Value.IndexOf("\") = -1 Then
				impsFile = EDLLocDr + iwkey.Value
			Else
				impsFile = iwkey.Value
			End If
			'===參數值檢查
		...
		If FileIO.FileSystem.FileExists(impsFile) = False Then
			Return CType("<<讀取失敗>> 來源檔不存在:" + impsFile, SqlString)
			Exit Function
		End If
		...
		Try
			If FileType.ToUpper = "TXT" Then
				...
				If Rtcode = "Schema OK!" Then
					If Mds.Tables.Count = 1 Then rcount = readTxtFile(Mds.Tables(0), impsFile)
					If Mds.Tables.Count > 1 Then rcount = readTxtFile(Mds, impsFile)
				Else
		...
		If rcount > 0 Then
			Try
				If INFType.ToUpper = "IMPORT" Then
					tscount = XsdMapCopy(XDoc, Mds)
					ReOK_Value = "<<XSD讀取 \N 筆，新增 \X 筆 TO " & SQLBulkCk & " !>>OK :"
				End If
		...
		Try
			...
			If SQLBulkCk <> "" Then
				tscountCk = DBConCls.ExcuteCount("SELECT count(*) from " + SQLBulkCk + " where etyps=0 and sfile='" + impsFile + "'")
			...
			If ODMTyps <> "" Then
				If ODMTyps.IndexOf("@wkey") > -1 Then ODMTyps = ODMTyps.Replace("@wkey", impsFile).Replace("@tscount", tscount)
				DBConCls.ExcuteSql(ODMTyps, DBConCls.CONNs)
			End If

			If tscountCk > tscount Then tscount = tscountCk
			...
			''接口匯入指定執行    BuName
			If BuName <> "" Then
				DBConCls.ExcuteSql(BuName, DBConCls.CONNs)
			End If
		Catch ex As Exception
			DBConCls.ExcMag = Now.ToString("yyyy/MM/dd hh:mm:ss") & vbTab & "{0}[{1}		,{2}	,{3}	,{4}]" & vbTab & "錯誤Message:" + ex.Message & vbCrLf
			FileIO.FileSystem.WriteAllText(DBConCls.APPLog, String.Format(DBConCls.ExcMag, "3.XsdBulkCopy(登記)", iwkey.Value, ODMTyps, JOBQTyps, ReOK_Value), True)
		End Try
	...
	'============================ 
	<Microsoft.SqlServer.Server.SqlFunction(DataAccess:=Microsoft.SqlServer.Server.DataAccessKind.Read)>
	Public Shared Function XsdQueryExp(ByVal ikey As SqlString, ByVal wkey As SqlString, ByVal iExtkeys As SqlString, ByVal InfXSD As SqlString) As SqlString
		Dim Mds As New DataSet
		...
		Try
			''取得 XSD 的接口程式設定
			Extkeys = iExtkeys.Value.Split(New Char() {","})
			Stkey = Extkeys(0)
			XDoc = XDocument.Load(InfXSD.Value)
			XApi = From el In XDoc.Descendants().<xsd:appinfo> Select el
			...
			End If
			''檔名規則
			Filename = FileFmtname(XApi, ikey.Value, Stkey, wkey.Value)
			'' TXT 格式
			If FileType.ToUpper = "TXT" Then
		...
		Try
			...
			CommandText = CommandText.Replace("@wkey", wkey.Value).Replace("@ExtkeyS", Stkey)
			...
			If FileType = "DsoFmtCK" Then
				ReOK_Value = DsoFmtCK(ikey, wkey, Stkey, InfXSD)
			End If
			If FileType = "DsoFmt7_11" Then
				ReOK_Value = DsoFmt7_11(ikey, wkey, Stkey, InfXSD)
			End If
			If FileType = "DsoFmtCVS" Then
				ReOK_Value = DsoFmtCVS(ikey, wkey, Stkey, InfXSD)
			End If
		Catch ex As Exception
	...
	Public Shared Sub EDIinfxSDView(ByVal ikey As SqlString, ByVal iwkey As SqlString, ByVal iStkey As SqlString, ByVal InfXSD As SqlString)
		...
		Catch ex As Exception
			ReOK_Value += String.Format("{0,15-}	{1,15-}	{2:s} ,ErrMessage: {3} ", "infXsd", "EDIinfxsdView數據結構解析錯誤", Now, ex.Message)
			FileIO.FileSystem.WriteAllText(DBConCls.APPLog, ReOK_Value + vbCrLf, True)
		End Try
		If rcount > 0 Then
			ReOK_Value = rcount.ToString + " 筆 XSD 結構傳送 SQL 測試 !"
			XsdTabView(Mds.Tables(0), ReOK_Value)
		End If
	...
	Public Shared Function FileFmtname(ByVal Apinfo As IEnumerable(Of XElement), ByVal infkey As String, ByVal exkey As String, ByVal wkey As String) As String
		...
		If Fmt.IndexOf("@") > 0 Then
			If Fmt.IndexOf("@stkey") > 0 Then Fmt = Fmt.Replace("@stkey", Right(Stkey, 4))
	...
	Public Shared Function JobQSvrCmd(ByVal CmdJobq As String, ByVal ewkey As String, ByVal setValue As String, ByVal WhereStr As String, Optional ByVal msgKey() As String = Nothing) As Integer
	  ...
	  + "from  EDIinf  where Infid='" + InfKey + "' and Stkey='" + Stkey + "'; "
		...
		Catch ex As DataException
			DBConCls.ExcMag = Now.ToString("yyyy/MM/dd hh:mm:ss") & vbTab & "{0}[{1}		,{2}	,{3}	,{4},{5}]" & vbTab & "錯誤Message:" + ex.Message & vbCrLf
			FileIO.FileSystem.WriteAllText(DBConCls.APPLog, String.Format(DBConCls.ExcMag, "JobQSvrCmd", CmdJobq, ewkey, setValue, WhereStr), True)
		End Try
	...
	Public Shared Function readTxtFile(ByRef tabN As DataTable, ByVal sfile As String) As Integer
		...
		Catch ex As Microsoft.VisualBasic.FileIO.MalformedLineException
			...
			FileIO.FileSystem.WriteAllText(DBConCls.APPLog, String.Format(" 錯誤描述  ERR: {0} ; FixedWidth  {1}  ", ReOK_Value, LarrWidth) + Chr(10), True)
		End Try
	End Function

	Public Shared Function readTxtFile(ByRef Dtabs As DataSet, ByVal sfile As String) As Integer
		readTxtFile = -1
		Dim lno As Integer = 0
		Dim wsH As Integer()
		Dim wsD As Integer()
		...
		' XDoc.Descendants.<xsd:appinfo>.<FileMap>.@Encoding.FirstOrDefault.Value()
		FieldParser = IIf(XApi.<EDIinf>.@FieldParser Is Nothing, XApi.<FileMap>.@FieldParser, XApi.<EDIinf>.@FieldParser)
		SearchStr = IIf(XApi.<FileMap>.@SearchStr Is Nothing, "", XApi.<FileMap>.@SearchStr).ToString.Split(",")
		If IsNumeric(FieldParser) Then FieldParser = ChrW(FieldParser)
			...
			Using myReader As New TextFieldParser(sfile, Filecode)
				...
				Select Case FieldParser
					Case "FixedWidth"
						'必需使用Bytes字元寬度
						myReader.TextFieldType = FieldType.FixedWidth
						For Each vt As DataTable In Dtabs.Tables
							wsH = (From el In MapInf Where el.fixed <> 0 And el.SourceDt = "a" Select el.fixed).ToArray
						...
						While Not myReader.EndOfData
							...
							If lno = 0 Then
								...
								TxtVal = myReader.ReadFields
								Dtabs.Tables(0).Rows.Add(TxtVal)
							Else
					...
					Case Else
						...
						While Not myReader.EndOfData
							...
							If IsNumeric(SearchStr(1)) = False Then
								For i = 1 To SearchStr.Length - 1
									If myReader.PeekChars(SearchStr(0)).IndexOf(FieldParser + SearchStr(i)) > 0 Then
										TxtVal = myReader.ReadFields
										Dtabs.Tables(i - 1).Rows.Add(TxtVal)
										lno += 1
									End If
								Next
							Else
								TxtVal = myReader.ReadFields
								If lno = 0 Then Dtabs.Tables(0).Rows.Add(TxtVal)
								If lno > 0 Then Dtabs.Tables(1).Rows.Add(TxtVal)
								If SearchStr.Length > 2 And myReader.EndOfData = True Then
									Dtabs.Tables(2).Rows.Add(TxtVal)
								End If
								lno += 1
							End If
		...
		Catch ex As Microsoft.VisualBasic.FileIO.MalformedLineException
			ReOK_Value += "ReadTxtFile<讀取 \N 筆發生錯誤 ! " & ex.Message & ">"
			FileIO.FileSystem.WriteAllText(DBConCls.APPLog, String.Format(" 錯誤描述  ERR: {0} ; readTxtFile  {1}  ", ReOK_Value, LarrWidth) + Chr(10), True)
		End Try
	End Function
	...
	Public Overloads Shared Function readXmlFile(ByVal sfile As String, ByRef infxsd As String, ByRef MidDs As DataSet) As Integer
		Try
			...
			If MidDs.Tables(0).Columns.Contains("sfile") = False Then
				...
				MidDs.Tables(0).Columns.Add("clientcode", Type.GetType("System.String"))
		...
		Try
			...
			For Each vt In Vxsd
				...
				If MidDs.Tables(vt.name).Columns.Contains("sfile") = False Then
					...
					MidDs.Tables(0).Columns("clientcode").DefaultValue = Stkey
				Else
					MidDs.Tables(vt.name).Columns("edate").DefaultValue = CDate(Now.ToString("yyyy/MM/dd HH:mm:ss"))
					MidDs.Tables(vt.name).Columns("sfile").DefaultValue = impsFile
					MidDs.Tables(0).Columns("etyps").DefaultValue = 0
		...
		For Each vt In Vxsd
			...
			If MidDs.Tables(vt.name).Rows.Count > 0 And SqlCopy.ColumnMappings.Count > 0 Then
				...
				Try
					...
					Rtrow += MidDs.Tables(vt.name).Rows.Count
				Catch ex As Exception
					XsdMapCopy = Now.ToString("yyyy/MM/dd hh:mm:ss") & vbTab & "{0}[{1}	,{2}	,{3}]" & vbTab & "SqlCopy錯誤Message:" + ex.Message & vbCrLf
					FileIO.FileSystem.WriteAllText(DBConCls.APPLog, String.Format(XsdMapCopy, "WriteToServer", SqlCopy.DestinationTableName, SqlCopy.ColumnMappings.Count, MidDs.Tables(vt.name).Rows.Count), True)
					'回傳 錯誤內容 暫無法傳到前端，改使用XML 記錄錯誤 
					MidDs.Tables(vt.name).Rows(0)("sfile") = ex.Message
					MidDs.WriteXml(impsFile.ToString.Replace(".XML", "") & "-內容錯誤協助.XML")
					SqlCopy.ColumnMappings.Clear()
				End Try
			End If
		Next
